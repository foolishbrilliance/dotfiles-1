...
CHECKS_PASS=false
if ! $(open -b com.sublimetext.2); then
...
if [ $CHECKS_PASS == true ]; then
  ...
  echo "Did nothing for SublimeText 2."
fi

echo "Configuring Dropbox sync for SublimeText3..."
SUBL_SETTINGS=~/Library/Application\ Support/Sublime\ Text\ 3/Packages/User
DB_SETTINGS=~/Dropbox/Documents/Application\ Files/SublimeText\ 3/User 
# Check if:
# 1) SublimeText is installed
# 2) Dropbox settings have been synced
# 3) Sublime settings exist
# 4) That these links have not been created before
CHECKS_PASS=false
if ! $(open -b com.sublimetext.3); then
  echo "SublimeText3 not installed. Exiting..."
elif [ ! -d "${DB_SETTINGS}" ]; then
  echo "${DB_SETTINGS} does not exist. Exiting..."
elif [ ! -d "${SUBL_SETTINGS}" ]; then
  echo "${SUBL_SETTINGS} does not exist. Exiting..."
...
if [ $CHECKS_PASS == true ]; then
  echo "Configuring Dropbox sync for SublimeText3. Backing up current settings to ${SUBL_SETTINGS}.orig"
  mv "${SUBL_SETTINGS}" "${SUBL_SETTINGS}.orig"
  ln -s "${DB_SETTINGS}" "${SUBL_SETTINGS}"
else
  echo "Did nothing for SublimeText 3."
fi
